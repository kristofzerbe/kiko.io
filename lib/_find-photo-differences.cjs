...
const tabletFolder = path.join(photoFolder, "tablet");
...
let tabletFiles = fs.readdirSync(tabletFolder).map(f => {
  return path.parse(f).name;
});
// console.log(tabletFiles);
...
console.log(magenta("NORMAL has no TABLET:"));
console.log(normalFiles.filter(x => !tabletFiles.includes(x)));