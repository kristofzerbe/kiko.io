...
const { getHelpers } = require("./tools.cjs");
...
class StatsCollector {
  ...
  getPostStats() {
    ...
    let postsSorted = _locals.posts.map(p => {
    ...
    /** LISTS --- ab NODE 21, Current v20.15.0 > NVM install */ 
    // const yearPosts = Object.groupBy(_locals.posts.map(p => { return p.date }), ({date}) => {
    //   return date.getFullYear();
    // });
    // console.log(yearPosts);

    // const posts = _locals.posts
    //   .map(
    //     ({ slug, title, source, path, permalink, date }) => 
    //     ({ slug, title, source, path, permalink, date })
    //   );
    // console.log(posts);
    return result;
