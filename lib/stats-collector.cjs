...
/** --------------------------------------------------------------------------------------
  ...
  getPostStats() {
    ...
    cnt.photos.unused.asset   = en.format(od.filter(([k, v]) => v.photo === true && v.status === "unused" && v.type === "asset").length);
    ...
    [...synPosts, ...synPhotos]
      .forEach(p => {
        let syn = p.syndication.map(s => { 
          return { site: s.site, url: s.url, type: p.type, item: p.slug } 
        });
    ...
    result.lists.syndicationsPerSite = synUnique
      .reduce((acc, s) => {
        acc[s.site] = acc[s.site] ? acc[s.site] + 1 : 1;
        return acc;
      },{});
    ...
    result.lists.photosPerCamera = Object.entries(_locals.dynamic)
      .filter(([k, v]) => v.photo === true && v.type !== "asset") //&& v.meta?.custom?.camera !== "Unknown")
      .map(([k, v]) => {
        return {
          name: k,
  ...
  getLinkStats() {
      ...
      .forEach(p => {
        ...
        dom.window.document.querySelectorAll("a").forEach(a => {
          ...
          if (a.href.startsWith(".") || a.href.startsWith("/")) {
            host = "kiko.io";
            if (a.href.startsWith("/photos/")) host = null;
          } else if (a.href.startsWith("http")) {
            try { 
              host = new URL(a.href).hostname;
            } catch (error) { console.log(">>> getLinkStats: " + error.code + ": " + error.input + " @ " + p.slug); }
          }

          if (host) {
            let domain = host.split(".");
            let tld = domain.at(-1);
            let sld = ["au","br","ca","id","il","in","jp","kr","ng","nz","pk","ru","th","tr","ua","uk","za"]; // https://en.wikipedia.org/wiki/Second-level_domain
            if (domain.length >= 3) {
              host = ((sld.includes(tld)) ? domain.slice(-3) : domain.slice(-2)).join(".");
            }

            if (links[host]) {
              if (!links[host].some(item => item.targetUrl === lnk.targetUrl)) {
                links[host].push(lnk);
              }
            } else {
              links[host] = [lnk];
    ...
    Object.entries(links).forEach(h => { 
      result.counts.linksAll += h.length; 
    });
    result.counts.hostsAll = Object.entries(links).length;
    result.counts.hostsOnce = Object.entries(links).filter(([k, v]) => v.length === 1).length;
    result.lists.hosts = Object.entries(links)
