  ...
  getPostStats() {
    ...
    [...synPosts, ...synPhotos]
      .forEach(p => {
        ...
        if (syn) {
          syndications.push(...syn);
          //syndicationSet.add(...syn);
        }
      });
    //let syndications = Array.from(syndicationSet);
    logobj(syndications);

    //De-Dupe syndications by type & url
    const synUnique = syndications.reduce((acc, s) => {
    ...
    result.lists.syndicationHostCount = synUnique
      .reduce((acc, s) => {
