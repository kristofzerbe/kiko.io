...
module.exports = {
  getPhoto: (hexo, photoName, that) => {
    ...
    if (that && that.asset_dir) {
      let assetDir = path.join(that.asset_dir, photoName + ".jpg");
      let assetPath = that.path;
      if (that.isLocale) {
        assetDir = assetDir.replace(".de\\", "\\");
        assetPath = that.__permalink.replace("/de", "/");
      }
      if (fs.existsSync(assetDir)) {
        item = {
          type: "asset",
          key: photoName,
          urlNormal: `/${assetPath}${photoName}.jpg`,
          urlMobile: `/${assetPath}${photoName}.jpg`,
          isPhoto: false
        } 
      }
    }
