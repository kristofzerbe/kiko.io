...
class TinyToolsGenerator {
  ...
  constructor() {
    ...
    _ttDir = path.join(_rootPath, _config.data_dir, _config.tiny_tools_dir);
    //console.log(_ttDir);

    _tt = fs
      .readdirSync(_ttDir)
      .filter((entry) => fs.statSync(path.join(_ttDir, entry)).isFile())
      .filter((entry) => !entry.startsWith(_config.tiny_tools_dir))
  ...
  generate() {
    ...
    _tt.forEach((tt) => {
      ...
      const regexImage = /(?:[!]\[(?<caption>.*?)\])\((?<image>.*?)\)/g;
      const resultImage = regexImage.exec(tt._content);
