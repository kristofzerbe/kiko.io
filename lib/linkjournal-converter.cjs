...
class LinkJournalConverter {
  ...
  constructor(yearMonth, poolImageName) {
    ...
    this.storageName = this.config.linkjournal.name_storage + " " + yearMonth;
    ...
    this.storageFolder = path.join(this.config.linkjournal.absolute_folder_storage, this.storageName);
    if (!fs.existsSync(this.storageFolder)) { fs.mkdirSync(this.storageFolder); }

    this.storageIndexFile = path.join(this.storageFolder, this.storageName + ".md");
    this.dataFile = path.join(_rootPath, this.config.data_dir, this.config.linkjournal.name_storage, this.storageName + ".json");
    ...
    let poolDir = path.join(_rootPath, this.config.static_dir, this.config.pool_dir);
    ...
    this.post = {
      ...
      photograph: getStaticPhotographInfo(poolDir, this.poolImageName, photosDir),
      count: 0,
      items: []
    }
  ...
  convert() {
    ...
    files.forEach(file => {
      ...
      if (fm.link) {
        ...
        cardlink[0].description = cardlink[0].description
          .replaceAll("<", "&lt;")
          .replaceAll("<", "&lt;")
    ...
    //Move {files} to {this.storageFolder}
    filesToMove.forEach(file => {
      let inFile = path.join(this.inboxFolder, file);
      let outFile = path.join(this.storageFolder, file)
      //console.log("MOVE " + inFile + " TO " + outFile);
      fs.renameSync(inFile, outFile);
    });

    //TEST: Create Index file
    fs.writeFile(this.storageIndexFile, "%% Waypoint %%", (err) => {
      if(err) { console.error(err); }
    });

    //TEST: Write links to data file
    delete this.post._folderPath;
    delete this.post._filePath
    delete this.post.photograph;
    fs.writeFile(this.dataFile, JSON.stringify(this.post), (err) => {
      if(err) { console.error(err); }
    });
  }
}
