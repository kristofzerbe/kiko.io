...
const { slugify, slugifyTitle, getDataFromCardlinkCodeBlock } = require("./tools.cjs");
...
class DiscoveriesGenerator {
  ...
  getItem(link) {
    ...
    let matchesAttachments = fm._content.matchAll(attachmentRegex);
    item.attachments = [...matchesAttachments].map(attachment => { 
      console.log(attachment);
      let a = this.getAttachment(attachment[1]);
      item.content = item.content.replace(`![[${attachment[1]}]]`, `![](/post/${_post.key}/${a.file})`);
      return a;
    });
