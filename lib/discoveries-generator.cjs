...
class Generator {
  ...
  constructor(discoveryNo, listName) {
    ...
    _post.list = listName;
    _post.key = "Discoveries-" + discoveryNo + listName.replace(/Discoveries|[^\w]/g, "-").replace(/-{2,10}/g, "-");
    _post.date = new Date().toISOString().replace(/T/, ' ').replace(/\..+/, '');
    _post.title = "Discoveries #" + discoveryNo + " - " + listName.replace(/Discoveries/g, " ").replace(/ {2,10}/g, " ");
