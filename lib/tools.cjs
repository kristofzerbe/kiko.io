...
module.exports = {  
  ...
  getCardLinkMatchContent(markdown, index) {
    const regex = /(?:[`]{3}cardlink\n.*?\n[`]{3})/gs;
    const matches = markdown.matchAll(regex);
    let match = [...matches][index].toString();
    //console.log(match);
    return match;
  },
  ...
  getStaticPhotographInfo: (staticDir, imageName, photosDir) => {
    let staticPath = path.join(staticDir, imageName);
    if (!fs.existsSync(path.join(staticPath, "meta.json"))) { throw "Photo (meta) not found"; }
    let photoMetaFile = fs.readFileSync(path.join(staticPath, "meta.json"));
    ...
    let photograph = {
      ...
      io: {
        meta: {
          in: path.join(staticPath, "meta.json"),
          out: path.join(photosDir, "meta", imageName + ".json")
        },
        mobile: {
          in: path.join(staticPath, "mobile.jpg"),
          out: path.join(photosDir, "mobile", imageName + ".jpg")
        },
        normal: {
          in: path.join(staticPath, "normal.jpg"),
          out: path.join(photosDir, "normal", imageName + ".jpg")
        }
      }
    };
    return photograph;
  ...
  getFullMonthAndYear: (yymmString) => {
    let arr = yymmString.split("-");
    let date = new Date(2000 + parseInt(arr[0]), parseInt(arr[1]) - 1, 1); // month start with 0 !?!
    return {
